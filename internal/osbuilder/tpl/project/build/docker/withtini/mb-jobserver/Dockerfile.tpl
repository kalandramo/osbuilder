...
# 构建（使用 make 产出可执行文件）
RUN make build BINS={{.Job.BinaryName}}
...
COPY --from=builder --chown=${UID}:${GID} /workspace/_output/platforms/linux/amd64/{{.Job.BinaryName}} /app/{{.Job.BinaryName}}

# 安全性：以非 root 用户运行
USER ${UID}:${GID}

# 以 tini 作为最小 init，正确处理信号与僵尸进程
ENTRYPOINT ["/usr/bin/tini", "--", "/app/{{.Job.BinaryName}}"]
