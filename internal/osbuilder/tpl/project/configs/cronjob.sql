...
(
    ...
    `started_at` datetime(3) DEFAULT NULL COMMENT 'Job 开始时间',
    `ended_at` datetime(3) DEFAULT NULL COMMENT 'Job 结束时间',
    `created_at`  timestamp    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`  timestamp    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
(
    ...
    `name`       varchar(191) DEFAULT NULL COMMENT 'LOCKS 名称',
    `owner_id`   longtext     DEFAULT NULL COMMENT '属主ID',
    `expired_at` datetime(3) DEFAULT NULL COMMENT '过期时间',
    `created_at` datetime(3) DEFAULT NULL COMMENT '创建时间',
    `updated_at` datetime(3) DEFAULT NULL COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uni_locks_name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Job锁表'
